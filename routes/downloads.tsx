...
export const handler: Handlers<GithubData | null> = {
    async GET(_, ctx) {
        const resp = await fetch("https://api.github.com/repos/Raspirus/Raspirus/releases");
        if (resp.status === 404) {
            ...
            <Header title="Downloads | Raspirus" />
                ...
                <div class="flex flex-wrap">
                    <Download title="Windows" image="img/windows.png" desc={desc} action={data?.win_link ?? ''} size={data?.win_size ?? 0}
                        action2={data?.win_msi_link ?? ''} size2={data?.win_msi_size ?? 0} />
                    <Download title="macOS" image="img/apple.png" desc={desc} action={data?.mac_link ?? ''} size={data?.mac_size ?? 0}
                        action2={null} size2={null} />
                    <Download title="Debian/Linux" image="img/linux.png" desc={desc} action={data?.linux_link ?? ''} size={data?.linux_size ?? 0}
                        action2={data?.linux_deb_link ?? ''} size2={data?.linux_deb_size ?? 0} />
