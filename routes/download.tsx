...
interface GithubData {
  ...
  linux_deb_riscv64_link: string;
  linux_deb_riscv64_size: number;
  ...
  async GET(_req, ctx) {
    try {
      ...
      const gdata = {
        ...
        mac_arm_link: json_res[0].assets.find((asset: Asset) =>
          asset.name.endsWith("_aarch64.dmg")
        )?.browser_download_url ?? "",
        mac_arm_size: json_res[0].assets.find((asset: Asset) =>
          asset.name.endsWith("_aarch64.dmg")
        )?.size ?? 0,
        ...
        linux_deb_riscv64_link: json_res[0].assets.find((asset: Asset) =>
          asset.name.includes("_riscv64.deb")
        )?.browser_download_url ?? "",
        linux_deb_riscv64_size: json_res[0].assets.find((asset: Asset) =>
          asset.name.includes("_riscv64.deb")
        )?.size ?? 0,
...
const Download = (props: PageProps<GithubData>) => {
  ...
  if (day !== undefined && month !== undefined && year !== undefined) {
    formattedDate = githubData?.date.toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "2-digit",
      year: "numeric",
    });
  ...
  return (
    <div>
      ...
      downloadLink2={data?.mac_arm_link}
      downloadSize2={data?.mac_arm_size}
      updateDate={formattedDate}
      latestVersion={data?.version}
      />
      <DownloadCard
      ...
      <DownloadCard
      id="raspberrypi"
      title="Debian/Linux (ARM 64)"
      imageName="raspberrypi.png"
      description="On Raspberry Pi and other single-board computers, we offer two deb packages, one for ARM64 and one for RISC-V64. If you don't know which one to choose,
        you can run the following command in the terminal: 'uname -m'. If the output is 'aarch64', you should choose the ARM64 package,
        if the output is 'riscv64', you should choose the RISC-V64 package. Cross-compiled from Ubuntu 24.04"
      downloadLink={data?.linux_deb_arm64_link}
      downloadSize={data?.linux_deb_arm64_size}
      downloadLink2={data?.linux_deb_riscv64_link}
      downloadSize2={data?.linux_deb_riscv64_size}
      updateDate={formattedDate}
      latestVersion={data?.version}
      />
      ...
      id="flathub"
      title="Flathub (Linux)"
      imageName="flathub.png"
      description="Flathub is a growing collection of apps for Linux. If you have Flatpak installed, you can install Raspirus with the following command:
        'flatpak install flathub io.github.raspirus.raspirus'."
      downloadLink="https://flathub.org/apps/io.github.raspirus.raspirus/"
      downloadSize={0}
      updateDate="Not provided"
      isExternal={true}
      />
